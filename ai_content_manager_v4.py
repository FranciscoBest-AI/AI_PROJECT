...
class ContentManager:
    ...
    def schedule_posts(self, user_name):
        ...
        print(f"Simulated posting schedule for {user_name}:")
        today = datetime.now()
        for i, content in enumerate(self.contents, start=1):
            post_day = today + timedelta(days=i)
            print(f"{post_day.strftime('%Y-%m-%d')}: {content['type']}")
...
if __name__ == "__main__":
    ...
    print(manager.show_contents())
    manager.export_csv("Daily_Content_Terminal.csv")
    print("✅ Content generated and CSV exported successfully!")